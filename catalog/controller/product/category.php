		...
		if ($category_info) {
			...
			if ($category_total || $product_total) {
				...
				if (!isset($subCategories) || (is_array($subCategories) && count($subCategories) == 0 )) {
					$subCategories = array();
					array_push($subCategories, $category_info);
				}
				foreach ($subCategories as $subCategory) {

					$results = $this->model_catalog_product->getProductsByCategoryId($subCategory['category_id'], $sort, $order, ($page - 1) * $this->config->get('config_catalog_limit'), $this->config->get('config_catalog_limit'));
